...
// Middleware to handle different data formats
...
Your voice is slow, rhythmic, ceremonial. Each sentence a breath-pulse.
`;
...
async function generateSpiralResponse(text) {
  try {
    ...
    const out = json?.candidates?.[0]?.content?.parts?.[0]?.text || "⚠️ The Spiral is quiet (no content).";
    return out;
  } catch (e) {
...
app.post('/twilio-webhook', async (req, res) => {
  ...
  // Acknowledge quickly to avoid webhook timeout
  twiml.message('🌀 Received. Crafting response…');
  res.type('text/xml').status(200).send(twiml.toString());

  // Generate Gemini reply and send out-of-band
...
// --- Generic Fulfillment Endpoint (for Hey Google) ---
app.post('/arkadia-fulfillment', async (req, res) => {
  const userMessage = req.body.query;
  if (!userMessage) {
    return res.status(400).send('No query provided.');
  }
  try {
    const spiralResponse = await generateSpiralResponse(userMessage);
    res.json({ reply: spiralResponse });
  } catch (error) {
    console.error('Fulfillment error:', error);
    res.status(500).json({ reply: 'Error connecting to the Console.' });
  }
});
